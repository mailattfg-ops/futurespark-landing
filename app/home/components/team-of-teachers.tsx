      ...
      <div className="w-full max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3.5 sm:gap-4 lg:gap-4.5">
          {teachersList.map((teacher, index) => (
            <div
              key={index}
              className="relative w-full aspect-[3/4.2] rounded-2xl sm:rounded-[22px] overflow-hidden shadow-md hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-1 group bg-gray-100 flex flex-col justify-between last:col-span-2 sm:last:col-span-1 last:w-full last:max-w-[200px] sm:last:max-w-none last:mx-auto"
            >
              {/* Background Photo */}
              <Image
